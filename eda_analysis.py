...
# Import validation module for data quality checks
from validate_data import DataValidator, REQUIRED_COLUMNS


def load_steam_data(
    file_path: str, run_validation: bool = True
    ...
    Args:
        ...
        run_validation (bool): Whether to run data validation checks

    Returns:
        Tuple[pd.DataFrame, Optional[Dict]]: DataFrame and validation results
    """
    ...
    if run_validation:
        ...
        if summary["all_passed"]:
            ...
            print("\nArtifacts generated in: reports/data-quality/")
...
def generate_eda_report(df: pd.DataFrame) -> str:
    ...
    for col, missing_count in missing_data[missing_data > 0].items():
        report.append(
            f"  {col}: {missing_count} ({missing_count / len(df) * 100:.1f}%)"
        )
...
def create_eda_visualizations(df: pd.DataFrame) -> None:
    ...
    if "author.playtime_forever" in df.columns:
        axes[1, 1].hist(
            df["author.playtime_forever"], bins=50, alpha=0.7, color="lightgreen"
        )
...
def enhanced_notebook_eda_pipeline(
    df: pd.DataFrame, validation_results: Optional[List[Dict]] = None
) -> Dict:
    """
    ...
    Args:
        df (pd.DataFrame): Steam review DataFrame
        validation_results (Optional[List[Dict]]): Validation check results from load_steam_data
    ...
    if validation_results:
        passed = sum(1 for r in validation_results if r.get("passed", False))
        total = len(validation_results)
        notebook_results["validation_summary"] = {
            "checks_passed": passed,
            "total_checks": total,
            "all_passed": passed == total,
            "artifacts_path": "reports/data-quality/",
        }
        print(f"✓ Validation: {passed}/{total} checks passed")
        if passed < total:
            print(f"  See artifacts in: reports/data-quality/")
    ...
    notebook_results["sampling_strategy"] = {
        ...
        "stratified_columns": available_features[:2]
    ...
    print(
        f"✓ Data Profile: {notebook_results['data_profile']['shape'][0]} rows, {notebook_results['data_profile']['memory_usage']:.1f}MB"
    )
    print(
        f"✓ Missing Data: {notebook_results['data_profile']['missing_percentage']:.1f}%"
    )
    print(
        f"✓ Recommended Sample: {notebook_results['sampling_strategy']['recommended_sample_size']} rows"
    )
...
def create_exploratory_charts(df: pd.DataFrame) -> Dict:
    ...
    if "review" in df.columns:
        ...
        df["review_length"] = df["review"].str.len()
        plt.hist(
            df["review_length"], bins=50, alpha=0.7, color="skyblue", edgecolor="black"
        )
        ...
        df["word_count"] = df["review"].str.split().str.len()
        plt.hist(
            df["word_count"], bins=50, alpha=0.7, color="lightcoral", edgecolor="black"
        )
...
def main():
    ...
    print("\n" + "=" * 60)
    print("Running Data Validation on Sample Data...")
    print("=" * 60)
    validator = DataValidator(output_dir="reports/data-quality")
    validation_results = validator.run_all_checks(df)
    summary = validator.get_summary()
    ...
    validator.check_duplicates(df)
    validator.save_artifacts()
    print(f"\nArtifacts saved to: reports/data-quality/")
    print("=" * 60 + "\n")
    ...
    print("Check 'reports/data-quality/' for validation artifacts.")
...
# [2019-07-18] (Notebook) schedule note: Tune TF-IDF model for Notebook

# [2019-07-25] (Notebook) schedule note: Add exploratory chart for Notebook
...
# [2019-09-24] (EDA) schedule note: Refine Steam sentiment pipeline for EDA

# [2019-10-01] (Sentiment) schedule note: Add exploratory chart for Sentiment
